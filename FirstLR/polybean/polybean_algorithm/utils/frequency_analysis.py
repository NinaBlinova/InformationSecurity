...
class FrequencyAnalysis:
    ...
    def print_similarity(self, real_key: dict[str, str]):
        similarity_count, similarity_percent = self.find_similarity(real_key)

        print(f'Similarity: {similarity_count} | {similarity_percent * 100:2f}%')

    def find_similarity(self, real_key: dict[str, str]) -> tuple[int, float]:
        real_keys = set(real_key.keys())
        possible_keys = set(self._possible_key.keys())

        similarity = 0
        for key in possible_keys & real_keys:
            if real_key[key] == self._possible_key[key]:
                similarity += 1

        return similarity, similarity / len(possible_keys)
